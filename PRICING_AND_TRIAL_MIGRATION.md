...
> **Scope:** Launch Monthly **$19.99**, 3-month **$59.99**, 1-year **$199**, with a **14-day free trial** for post-launch new accounts only, while grandfathering existing monthly payers until their next renewal.  
...
| Rule | Behavior |
|------|----------|
| **Monthly price** | **$19.99** / month (new subscribers) |
| **Quarterly price** | **$59.99** / 3 months |
| **Annual price** | **$199** / year |
| **Free trial** | **14 days**, only for **brand-new accounts** created **on or after** `SUBSCRIPTION_TRIAL_LAUNCH_AT` who have **never** had any subscription (Stripe or Apple) |
...
| New monthly | $19.99 | `interval: month`, `interval_count: 1` | New checkouts |
| Quarterly | $59.99 | `interval: month`, `interval_count: 3` | 3-month billing |
| Annual | $199.00 | `interval: year`, `interval_count: 1` | Yearly billing |
...
|---------|----------|------------|----------------------|
| Pro Monthly | 1 month | $19.99 (after increase) | `com.eklan.ai.pro.monthly` (existing) |
| Pro Quarterly | 3 months | $59.99 | `com.eklan.ai.pro.quarterly` (new) |
| Pro Annual | 1 year | $199 | `com.eklan.ai.pro.annual` (new) |
...
3. **Schedule monthly price increase** to $19.99:
...
4. **Introductory offer:** Configure **14-day free trial** on monthly (and optionally quarterly/annual). Apple enforces once-per-group-per-Apple-ID eligibility automatically.
...
**Goal:** New customers pick Monthly / Quarterly / Annual; eligible users get 14-day trial.
...
- Plan picker: Monthly **$19.99** / 3-month **$59.99** / 1-year **$199**.
- Show **"14-day free trial"** only when `eligibleForTrial === true`.
- Old free / former subscribers: **"Subscribe"** + price, no trial copy.
- Pass `billingPeriod` in checkout POST body.

**Also fix:** [`src/app/(student)/account/payment/modal/page.tsx`](src/app/(student)/account/payment/modal/page.tsx) — stub says "7 days"; align with 14-day gated trial or remove.
...
**Goal:** Soft grandfather — current paid month untouched; next invoice uses $19.99.
...
Existing monthly payers keep old price until `current_period_end`; next renewal bills $19.99.

---

## 7. PR 4 — iOS App Store Connect + backend

**Goal:** iOS supports three durations + gated 14-day trial; price increase preserved for existing subscribers (Phase 0 ASC work).
...
2. Full renewal price (trial: show 14 days free **and** post-trial price).
...
|----------|---------|---------|
| `STRIPE_PREMIUM_MONTHLY_PRICE_ID` | `price_...` | **New** monthly $19.99 — new checkouts |
| `STRIPE_PREMIUM_MONTHLY_PRICE_ID_LEGACY` | `price_...` | **Old** monthly — grandfathered subs + billing-period mapping |
| `STRIPE_PREMIUM_QUARTERLY_PRICE_ID` | `price_...` | 3-month $59.99 |
| `STRIPE_PREMIUM_ANNUAL_PRICE_ID` | `price_...` | Annual $199 |
