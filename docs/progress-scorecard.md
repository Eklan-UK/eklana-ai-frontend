...
|----------|------------------|----------------|------------------|
| **Clarity** (API field: `pronunciation`) | How clearly words and sentences are pronounced, including medical terminology. | The average of Speechace scores across all completed drills. | |
...
| **Confidence** | Learner's overall communication effectiveness based on clarity, accuracy, and fluency. | The average of the Clarity, Accuracy, and Fluency scores above. | |
...
```
Clarity = average(Speechace scores from all completed drills)
```
...
```
Confidence = (Clarity + Accuracy + Fluency) / 3
```
...
|--------|------------------------|
| Clarity | All drills evaluated by Speechace |
...
- Empty pillars (no scored activity yet) show `0` on the dashboard cards and are excluded from the Confidence divisor. For example, a learner with only Clarity data will have `Confidence = Clarity`.
- Learner-facing label is **Clarity**; the API/storage field remains `pronunciation` for backwards compatibility.
