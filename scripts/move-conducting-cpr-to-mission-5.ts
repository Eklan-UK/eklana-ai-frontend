/**
 * Move `conducting_cpr` drills and prompt templates to Mission 5 (Bonus Scenarios).
 *
 * Sets `learning_journey_part: 5` (drills) / `part: "5"` (prompt templates) where
 * the topic is `conducting_cpr` and the part is not already 5.
 *
 * Safe by default: runs as a DRY RUN unless `--apply` is passed.
 *
 * Usage:
 *   npx tsx scripts/move-conducting-cpr-to-mission-5.ts                # dry run (default)
 *   npx tsx scripts/move-conducting-cpr-to-mission-5.ts --apply         # persist changes
 *   npx tsx scripts/move-conducting-cpr-to-mission-5.ts --verbose       # per-document detail
 */
