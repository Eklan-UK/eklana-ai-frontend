...
const DRILL_TYPE_OPTIONS = [
  ...
  { value: "key_phrases", label: "Scenario Pressure Test" },
] as const;
...
function buildPath(pagePath: string, filters: BookmarkedFilters): string {
...
export function BookmarkedDrillsPageContent({ basePath }: { basePath: string }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const pagePath = `${basePath}/bookmarked`;
  ...
  const updateFilters = useCallback(
    (partial: Partial<BookmarkedFilters>) => {
      ...
      router.replace(buildPath(pagePath, next), { scroll: false });
    },
    [filters, pagePath, router]
  );
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Library of bookmarked drills, filterable by mission, topic, and type
          </p>
        </div>
      </div>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Link
                          href={`${basePath}/create?drillId=${drill._id}`}
...
export function BookmarkedDrillsPage({ basePath }: { basePath: string }) {
  return (
    <Suspense
      fallback={
    ...
    >
      <BookmarkedDrillsPageContent basePath={basePath} />
    </Suspense>
  );
}
