  ...
  return (
    <button
      ...
      className={`w-full bg-white border border-gray-200 rounded-2xl p-4 flex items-center gap-4 hover:shadow-md hover:border-emerald-200 transition-all text-left ${
        !isUnlocked ? "opacity-60 cursor-not-allowed hover:shadow-none hover:border-gray-200" : ""
      }`}
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <p className="text-base font-bold font-nunito text-gray-900 truncate">{title}</p>
      ...
      <div className="flex items-center justify-center flex-shrink-0 w-8 h-8 rounded-full bg-gray-100">
        {isUnlocked ? (
...
function ScoreChip({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div className="flex flex-col items-center px-3 py-1.5 rounded-xl bg-white border border-gray-100">
        ...
        <div className="px-4 pb-4 border-t border-gray-100 pt-3 space-y-3">
          ...
          <div>
            <div className="flex justify-between mb-1">
              <span className="text-xs text-gray-500">Progress to Level {session.levelAfter + 1}</span>
              <span className="text-xs font-semibold text-violet-600">{Math.round(session.progressToNextLevel)}%</span>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-md mx-auto px-5 pb-12 md:max-w-2xl md:px-8">
        ...
        {hasHistory && history && (
          <div className="flex items-center justify-between bg-white border border-gray-200 rounded-2xl px-4 py-3 mb-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-base font-bold text-gray-900">Level {history.currentLevel}</p>
              </div>
            </div>
            {history.averages && (
              <div className="flex items-center gap-1 text-xs text-gray-500">
                <Zap className="w-3.5 h-3.5 text-blue-500" />
                <span className="font-semibold text-gray-700">{history.averages.responseSpeed.toFixed(1)}s</span>
                <span>avg speed</span>
              </div>
            )}
          </div>
        ...
        {activeTab === "practice" && (
          <div>
            <h1 className="text-sm text-gray-500 mb-1">What would you like to practice today?</h1>
        ...
        {activeTab === "history" && (
          <div>
            {historyLoading ? (
              <div className="space-y-3">
                {[0, 1].map((i) => (
                  <div key={i} className="bg-white border border-gray-200 rounded-2xl p-4 h-16 animate-pulse" />
                ))}
              </div>
            ) : !hasHistory ? (
