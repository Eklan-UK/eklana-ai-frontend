...
export default function AIPracticePage() {
  ...
  const [isInitializing, setIsInitializing] = useState(isDrillPractice);
  const [drillInfo, setDrillInfo] = useState<{ drillType: string; drillTitle: string } | null>(null);
  const [messages, setMessages] = useState<ChatMessage[]>(
    isDrillPractice
      ...
      : [
        {
          type: "ai",
          text: "Hey! I'm here to help you practice English. What would you like to talk about?",
          audioBase64: null,
          audioMimeType: null,
        },
      ]
  ...
  useEffect(() => {
    if (isDrillPractice && drillId && !drillInitRef.current) {
  ...
  useEffect(() => {
    if (!isDrillPractice && messages[0]?.type === "ai" && autoPlayAudio) {
      setTimeout(() => {
            ...
            {showMenu && (
              <>
                ...
                <div className="absolute right-0 top-10 z-40 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-1 animate-in fade-in slide-in-from-top-1 duration-150">
                  ...
                  <button
                    onClick={() => { router.push("/account/practice/ai/completed"); setShowMenu(false); }}
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
          ...
          {messages.map((message, index) => {
            ...
            return isAI ? (
              /* ── AI Message (matches mobile AIMessage.tsx) ──────────── */
              ...
              /* ── User Message (matches mobile UserMessage.tsx) ──────── */
