    ...
    <div className="relative space-y-6 pb-24">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Sessions you&apos;re enrolled in — join when the link is available
        </p>
      </div>
        ...
        >
          <button
            ...
            className={`flex min-w-0 flex-1 items-center justify-center gap-2 rounded-full py-2.5 pl-3 pr-2 text-sm transition-colors ${
              tab === "today" ? "text-gray-900" : "text-gray-500 hover:text-gray-700"
            }`}
          >
            <Clock
              className={`h-4 w-4 shrink-0 ${tab === "today" ? "text-gray-900" : "text-gray-400"}`}
              strokeWidth={2}
            />
            <span className={tab === "today" ? "font-bold text-gray-900" : "font-medium text-gray-500"}>
              Today
            </span>
          ...
          <button
            ...
            className={`flex min-w-0 flex-1 items-center justify-center gap-2 rounded-full py-2.5 pl-2 pr-3 text-sm transition-colors ${
              tab === "upcoming" ? "text-gray-900" : "text-gray-500 hover:text-gray-700"
            }`}
          >
            <Calendar
              className={`h-4 w-4 shrink-0 ${tab === "upcoming" ? "text-gray-900" : "text-gray-400"}`}
              strokeWidth={2}
            />
            <span className={tab === "upcoming" ? "font-bold text-gray-900" : "font-medium text-gray-500"}>
              Upcoming
            </span>
            <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-bold tabular-nums text-gray-900">
              {upcomingCount}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {!isLoading && visibleClasses.length === 0 ? (
          <div className="col-span-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-12 text-center text-sm text-gray-600">
            No classes in this tab. When your school schedules a class for you, it
            will show up here.
          </div>
        ) : null}
        {!isLoading &&
          visibleClasses.map((session) => {
            ...
            const heading = session.title?.trim() || session.studentLabel;
            ...
            return (
              ...
              >
                <div className="mb-2 flex items-start justify-between gap-3">
                  <h2 className="text-base font-bold leading-tight text-slate-900">{heading}</h2>
                  <StatusBadge status={session.status} />
                </div>

                <p className="mb-3 text-xs font-medium text-gray-500">Tutor: {session.tutorName}</p>
