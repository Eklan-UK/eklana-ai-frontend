...
async function handler(
	...
	try {
		// Only tutors and admins can parse documents
		if (context.userRole !== 'tutor' && context.userRole !== 'admin') {
			return NextResponse.json(
				{
					code: 'AuthorizationError',
					message: 'Only tutors and admins can parse documents',
				},
				{ status: 403 }
			);
		}
		...
		if (file instanceof File) {
			...
			if (blobLike && typeof blobLike.size === 'number' && typeof blobLike.arrayBuffer === 'function') {
		...
		if (error.message.includes('pdf-parse') || error.message.includes('mammoth') || error.message.includes('xlsx')) {
