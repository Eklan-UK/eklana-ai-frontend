...
function ReviewModal({
  ...
  const sentences = sentenceResults.sentences || [];
  ...
  >(() => {
    if (isAlreadyReviewed && existingReviews.length > 0) {
      return sentences.map((_, idx) => {
        const existing = existingReviews.find((r) => r.sentenceIndex === idx);
        return {
    ...
    return sentences.map(() => ({ isCorrect: null, correctedText: "" }));
  });

  const [isSubmitting, setIsSubmitting] = useState(false);

  const canSubmit = reviews.every((r) => r.isCorrect !== null);

  const handleSubmit = async () => {
    if (!canSubmit) {
      toast.error("Please review all sentences before submitting.");
      return;
    }

    // Validate that corrections are provided for incorrect sentences
    const incorrectWithoutCorrection = reviews.some(
      (r) => r.isCorrect === false && !r.correctedText.trim()
    );
    if (incorrectWithoutCorrection) {
    ...
    try {
      const sentenceReviews = reviews.map((r, idx) => ({
        sentenceIndex: idx,
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-blue-50 rounded-xl p-4">
            <h3 className="text-lg font-bold text-gray-900 mb-2">
              Target Word: <span className="text-blue-600">{sentenceResults.word}</span>
            </h3>
            <div className="bg-white rounded-lg p-3">
              <p className="text-xs text-gray-500 mb-1">Student's Definition:</p>
              <p className="text-gray-900">{sentenceResults.definition || "No definition provided"}</p>
            </div>
          </div>
