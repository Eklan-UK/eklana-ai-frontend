...
import { ArrowLeft, FileText, Plus, X, Calendar as CalendarIcon, ChevronDown, Loader2 } from 'lucide-react';
...
const DrillBuilder: React.FC = () => {
  ...
  // Sentence Writing
  const [sentenceWritingItems, setSentenceWritingItems] = useState<SentenceWritingItem[]>([
    { word: '', hint: '' }
  ]);
  ...
  const addVocabularyItem = () => {
    setVocabularyItems([...vocabularyItems, { word: '', wordTranslation: '', text: '', translation: '' }]);
  };
  ...
  const removeDialogueTurn = (sceneIndex: number, turnIndex: number) => {
    ...
    if (scenes[sceneIndex].dialogue.length > 2) {
      scenes[sceneIndex].dialogue = scenes[sceneIndex].dialogue.filter((_, i) => i !== turnIndex);
      setRoleplayScenes(scenes);
    }
  };
  ...
  const addMatchingPair = () => {
    setMatchingPairs([...matchingPairs, { left: '', right: '', leftTranslation: '', rightTranslation: '' }]);
  };
  ...
  const updateDefinitionItem = (index: number, field: keyof DefinitionItem, value: string) => {
  ...
  const updateSentenceWritingItem = (index: number, field: keyof SentenceWritingItem, value: string) => {
  ...
  const handleApplyParsedContent = () => {
    ...
    switch (type) {
      case 'vocabulary':
        setVocabularyItems(items.length > 0 ? items : [{ word: '', wordTranslation: '', text: '', translation: '' }]);
        break;
      case 'matching':
        setMatchingPairs(items.length > 0 ? items : [{ left: '', right: '', leftTranslation: '', rightTranslation: '' }]);
        break;
      case 'roleplay':
        setRoleplayScenes(items.length > 0 ? items : [{
          scene_name: 'Scene 1',
          dialogue: [{ speaker: 'ai_0', text: '', translation: '' }, { speaker: 'student', text: '', translation: '' }],
        }]);
        break;
      case 'definition':
        ...
        break;
      case 'grammar':
        setGrammarItems(items.length > 0 ? items : [{ pattern: '', example: '', hint: '' }]);
        break;
      case 'sentence_writing':
        setSentenceWritingItems(items.length > 0 ? items : [{ word: '', hint: '' }]);
        break;
      case 'summary':
  ...
  const handleSubmit = async () => {
    ...
    if (drillType === 'vocabulary') {
      if (vocabularyItems.length === 0 || vocabularyItems.some(v => !v.text.trim())) {
      ...
      if (roleplayScenes.length === 0 || roleplayScenes.some(s => s.dialogue.length < 2 || s.dialogue.some(d => !d.text.trim()))) {
        toast.error('Please add at least one scene with complete dialogue');
        return;
      }
    } else if (drillType === 'matching') {
      if (matchingPairs.length === 0 || matchingPairs.some(p => !p.left.trim() || !p.right.trim())) {
        toast.error('Please add at least one matching pair with both sides filled');
        return;
      }
    } else if (drillType === 'definition') {
      if (definitionItems.length === 0 || definitionItems.some(d => !d.word.trim())) {
        toast.error('Please add at least one word for definition');
        return;
      }
    } else if (drillType === 'grammar') {
      if (grammarItems.length === 0 || grammarItems.some(g => !g.pattern.trim())) {
        toast.error('Please add at least one grammar pattern');
        return;
      }
    } else if (drillType === 'sentence_writing') {
      if (sentenceWritingItems.length === 0 || sentenceWritingItems.some(s => !s.word.trim())) {
    ...
    try {
      ...
      const response = await drillAPI.create(drillData);
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex items-center gap-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Create New Drill</h1>
          <p className="text-gray-500 text-sm">Upload a PDF or fill in the form manually</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        <div className="xl:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            ...
            <div className="space-y-4 mt-6">
              {/* File Upload */}
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-2">Upload Document</label>
                <FileUploadZone
              ...
              {/* Clipboard Paste */}
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-2">Or Paste from Clipboard</label>
                <ClipboardPaste
              ...
              <div className="space-y-6">
                {vocabularyItems.map((item, idx) => (
                  <div key={idx} className="p-6 bg-gray-50/50 rounded-2xl relative border border-gray-100">
                    ...
                    <h4 className="text-sm font-bold text-gray-900 mb-4">Item {idx + 1}</h4>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Vocabulary Word (optional)</label>
                        <input 
                          type="text"
                          value={item.word || ''}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Word Translation (optional)</label>
                        <input 
                          type="text"
                          value={item.wordTranslation || ''}
                          onChange={(e) => updateVocabularyItem(idx, 'wordTranslation', e.target.value)}
                          placeholder="e.g. 식당"
                          className="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Practice Sentence<span className="text-red-500">*</span></label>
                        <input 
                          type="text"
                          value={item.text}
                          onChange={(e) => updateVocabularyItem(idx, 'text', e.target.value)}
                          placeholder="e.g. I went to the restaurant yesterday"
                          className="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Sentence Translation (optional)</label>
                        <input 
                          type="text"
                          value={item.translation || ''}
                          onChange={(e) => updateVocabularyItem(idx, 'translation', e.target.value)}
          ...
          {drillType === 'roleplay' && (
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm space-y-6">
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1.5">Context / Scenario<span className="text-red-500">*</span></label>
                <textarea 
                  value={context}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1.5">Student Character Name<span className="text-red-500">*</span></label>
                <input 
                  type="text"
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-xs font-bold text-gray-600">AI Character Names<span className="text-red-500">*</span></label>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-base font-bold text-gray-900">Roleplay Scenes<span className="text-red-500">*</span></h3>
                  <button
                ...
                <div className="space-y-4">
                  {roleplayScenes.map((scene, sceneIdx) => (
                    <div key={sceneIdx} className="p-6 bg-purple-50/30 rounded-2xl border border-purple-100">
                      <div className="flex items-center justify-between mb-4">
                        <input 
                          type="text"
                      ...
                      <div className="mb-4">
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Scene Context (optional)</label>
                        <input 
                          type="text"
                          value={scene.context || ''}
                          onChange={(e) => {
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-2">
                          <label className="text-xs font-bold text-gray-600">Dialogue</label>
                          <button
                        ...
                        <div className="space-y-3">
                          {scene.dialogue.map((turn, turnIdx) => (
                            <div key={turnIdx} className={`p-4 rounded-xl border ${
                              turn.speaker === 'student' ? 'bg-blue-50/30 border-blue-100' : 'bg-purple-50/30 border-purple-100'
                            }`}>
                              <div className="flex items-center justify-between mb-2">
                                <select
                                  value={turn.speaker}
                                  onChange={(e) => {
                                    const scenes = [...roleplayScenes];
                                    scenes[sceneIdx].dialogue[turnIdx].speaker = e.target.value;
                                    setRoleplayScenes(scenes);
                                  }}
                                  className="px-3 py-1 bg-white border border-gray-100 rounded-lg text-xs"
                                >
                                  <option value="student">{studentCharacterName || 'Student'}</option>
                                  {aiCharacterNames.map((name, aiIdx) => (
                                    <option key={aiIdx} value={`ai_${aiIdx}`}>{name || `AI ${aiIdx + 1}`}</option>
                                  ))}
                                </select>
                                {scene.dialogue.length > 2 && (
                                  <button
                                    onClick={() => removeDialogueTurn(sceneIdx, turnIdx)}
                              ...
                              <input 
                                ...
                                onChange={(e) => {
                                  const scenes = [...roleplayScenes];
                                  scenes[sceneIdx].dialogue[turnIdx].text = e.target.value;
                                  setRoleplayScenes(scenes);
                                }}
                              ...
                              <input 
                                ...
                                onChange={(e) => {
                                  const scenes = [...roleplayScenes];
                                  scenes[sceneIdx].dialogue[turnIdx].translation = e.target.value;
                                  setRoleplayScenes(scenes);
                                }}
          ...
          {drillType === 'matching' && (
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-bold text-gray-900">Matching Pairs<span className="text-red-500">*</span></h2>
                <button 
                ...
                {matchingPairs.map((pair, idx) => (
                  <div key={idx} className="p-6 bg-gray-50/50 rounded-2xl relative border border-gray-100">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-3">
                        <div>
                          <label className="block text-xs font-bold text-gray-600 mb-1.5">Left Side<span className="text-red-500">*</span></label>
                          <input 
                            type="text"
                            value={pair.left}
                        ...
                        <div>
                          <label className="block text-xs font-bold text-gray-600 mb-1.5">Left Translation (optional)</label>
                          <input 
                            type="text"
                            value={pair.leftTranslation || ''}
                            onChange={(e) => updateMatchingPair(idx, 'leftTranslation', e.target.value)}
                      ...
                      <div className="space-y-3">
                        <div>
                          <label className="block text-xs font-bold text-gray-600 mb-1.5">Right Side<span className="text-red-500">*</span></label>
                          <input 
                            type="text"
                            value={pair.right}
                        ...
                        <div>
                          <label className="block text-xs font-bold text-gray-600 mb-1.5">Right Translation (optional)</label>
                          <input 
                            type="text"
                            value={pair.rightTranslation || ''}
                            onChange={(e) => updateMatchingPair(idx, 'rightTranslation', e.target.value)}
          ...
          {drillType === 'definition' && (
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-bold text-gray-900">Definition Items<span className="text-red-500">*</span></h2>
                <button 
              ...
              <div className="space-y-6">
                {definitionItems.map((item, idx) => (
                  <div key={idx} className="p-6 bg-gray-50/50 rounded-2xl relative border border-gray-100">
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Word / Expression<span className="text-red-500">*</span></label>
                        <input 
                          type="text"
                          value={item.word}
                          onChange={(e) => updateDefinitionItem(idx, 'word', e.target.value)}
                          placeholder="e.g. Abundant"
                          className="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Hint (optional)</label>
                        <input 
                          type="text"
                          value={item.hint || ''}
                          onChange={(e) => updateDefinitionItem(idx, 'hint', e.target.value)}
          ...
          {drillType === 'grammar' && (
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-bold text-gray-900">Grammar Patterns<span className="text-red-500">*</span></h2>
                <button 
              ...
              <div className="space-y-6">
                {grammarItems.map((item, idx) => (
                  <div key={idx} className="p-6 bg-gray-50/50 rounded-2xl relative border border-gray-100">
                    ...
                    <h4 className="text-sm font-bold text-gray-900 mb-4">Pattern {idx + 1}</h4>
                    <div className="space-y-4">
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Grammar Pattern<span className="text-red-500">*</span></label>
                        <input 
                          type="text"
                          value={item.pattern}
                          onChange={(e) => updateGrammarItem(idx, 'pattern', e.target.value)}
                          placeholder="e.g. Used to + verb"
                          className="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-gray-600 mb-1.5">Example Sentence (optional)</label>
                        <input 
                          type="text"
                          value={item.example || ''}
                          onChange={(e) => updateGrammarItem(idx, 'example', e.target.value)}
                      ...
                      <div>
                        ...
                        <input 
                          type="text"
                          value={item.hint || ''}
                          onChange={(e) => updateGrammarItem(idx, 'hint', e.target.value)}
          ...
          {drillType === 'sentence_writing' && (
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-lg font-bold text-gray-900">Words for Sentence Writing<span className="text-red-500">*</span></h2>
                <button 
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input 
                          type="text"
                          value={item.word}
                          onChange={(e) => updateSentenceWritingItem(idx, 'word', e.target.value)}
                      ...
                      <div>
                        ...
                        <input 
                          type="text"
                          value={item.hint || ''}
                          onChange={(e) => updateSentenceWritingItem(idx, 'hint', e.target.value)}
          ...
          {drillType === 'summary' && (
            <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
              <h2 className="text-lg font-bold text-gray-900 mb-6">Article for Summary<span className="text-red-500">*</span></h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1.5">Article Title<span className="text-red-500">*</span></label>
                  <input 
                    type="text"
                    value={articleTitle}
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1.5">Article Content<span className="text-red-500">*</span></label>
                  <textarea 
                    ...
                    className="w-full px-4 py-3 bg-white border border-gray-100 rounded-xl"
                  />
                  <p className="text-xs text-gray-400 mt-2">Copy and paste the news article text for students to read and summarize.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="space-y-8">
           {/* Drill Settings */}
          <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            <h2 className="text-lg font-bold text-gray-900 mb-8">Drill Settings</h2>
            
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                   <label className="block text-xs font-bold text-gray-600 mb-1.5">Drill title<span className="text-red-500">*</span></label>
                   <input
                     type="text"
                     value={drillTitle}
                ...
                <div className="col-span-2">
                   <label className="block text-xs font-bold text-gray-600 mb-1.5">Completion Date<span className="text-red-500">*</span></label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                   <label className="block text-xs font-bold text-gray-600 mb-1.5">Drill type<span className="text-red-500">*</span></label>
                   <div className="relative">
                      <select 
                        value={drillType}
                ...
                <div>
                   <label className="block text-xs font-bold text-gray-600 mb-1.5">Difficulty<span className="text-red-500">*</span></label>
                   <div className="relative">
                      <select 
                        value={difficulty}
              ...
              {drillType !== 'roleplay' && (
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1.5">Context (Optional)</label>
                  <textarea 
                    value={context}
                    onChange={(e) => setContext(e.target.value)}
              ...
              <div>
                 <label className="block text-xs font-bold text-gray-600 mb-1.5">Example Pronunciation URL (Optional)</label>
                 <input
                   type="url"
