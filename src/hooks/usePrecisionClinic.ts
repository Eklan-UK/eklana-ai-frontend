...
export function useMovePrecisionClinicStudentWeekDrills(studentId: string) {
	const queryClient = useQueryClient();
	...
	return useMutation({
		...
		}) => precisionClinicAPI.moveStudentWeekDrills(studentId, data),
...
export function useDeletePrecisionClinicStudentWeeks(studentId: string) {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: (data: { weekNumbers: number[] }) =>
			precisionClinicAPI.deleteStudentWeeks(studentId, data),
		onSuccess: async (response) => {
			await invalidatePrecisionClinicStudentWeeks(queryClient, studentId);
			await Promise.all([
				queryClient.invalidateQueries({
