...
function formatReminderLabel(minutesBefore: number): string {
  if (minutesBefore === 60) return '1 hour';
  return `${minutesBefore} minute${minutesBefore === 1 ? '' : 's'}`;
}

export const sendClassReminderEmail = async (data: {
  studentEmail: string;
  studentName: string;
  classTitle: string;
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #1f2937; margin: 0; padding: 0; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="background-color: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06);">
      <div style="background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; padding: 30px; text-align: center;">
        <div style="font-size: 48px; margin-bottom: 10px;">🎓</div>
        <h1 style="margin: 0; font-size: 24px; font-weight: 600;">Class Starting Soon!</h1>
        <p style="margin: 8px 0 0 0; font-size: 16px; opacity: 0.9;">Your class begins in ${label}</p>
      </div>
      <div style="padding: 30px;">
        <p style="margin: 0 0 20px 0; font-size: 16px;">Hi <strong>${data.studentName}</strong>,</p>
        <p style="margin: 0 0 25px 0; font-size: 16px; color: #4b5563;">This is a reminder that your upcoming class is starting soon.</p>
        <div style="background-color: #f0fdf4; border: 2px solid #bbf7d0; border-radius: 12px; padding: 20px; margin: 25px 0;">
          <h2 style="margin: 0 0 12px 0; font-size: 18px; color: #166534;">${data.classTitle}</h2>
        ...
        <p style="margin: 25px 0 0 0; font-size: 14px; color: #6b7280; text-align: center;">Good luck! 🌟</p>
      </div>
    </div>
    <div style="text-align: center; margin-top: 30px; color: #9ca3af; font-size: 12px;">
      <p style="margin: 0;">This is an automated notification from Eklan.</p>
...
This is an automated notification from Eklan.
    ...
    await sendEmail({ to: data.studentEmail, subject, html, text });
    ...
    logger.error('Error sending class reminder email', {
      error: error.message,
      studentEmail: data.studentEmail,
    });
...
export const sendNpsFormEmail = async (data: {
  studentEmail: string;
  studentName: string;
  classTitle: string;
  formName: string;
  ...
  try {
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000';
    const subject = 'How was your class today?';
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #1f2937; margin: 0; padding: 0; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    ...
    <div style="background-color: white; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1), 0 2px 4px -1px rgba(0,0,0,0.06);">
      <div style="background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; padding: 30px; text-align: center;">
        <div style="font-size: 48px; margin-bottom: 10px;">💬</div>
        <h1 style="margin: 0; font-size: 24px; font-weight: 600;">We'd love your feedback!</h1>
        <p style="margin: 8px 0 0 0; font-size: 16px; opacity: 0.9;">Tell us about your recent class</p>
      </div>
      <div style="padding: 30px;">
        <p style="margin: 0 0 20px 0; font-size: 16px;">Hi <strong>${data.studentName}</strong>,</p>
        <p style="margin: 0 0 25px 0; font-size: 16px; color: #4b5563;">Thank you for attending <strong>${data.classTitle}</strong>. Your feedback helps us improve your learning experience.</p>
        <div style="background-color: #f0fdf4; border: 2px solid #bbf7d0; border-radius: 12px; padding: 20px; margin: 25px 0;">
          <h2 style="margin: 0 0 12px 0; font-size: 18px; color: #166534;">${data.formName}</h2>
          <p style="margin: 0; font-size: 14px; color: #374151;">Please take a moment to share your thoughts.</p>
        </div>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${data.formUrl}" style="display: inline-block; background: linear-gradient(135deg, #22c55e 0%, #16a34a 100%); color: white; text-decoration: none; padding: 14px 32px; border-radius: 8px; font-size: 16px; font-weight: 600; box-shadow: 0 4px 6px -1px rgba(34,197,94,0.4);">Share Feedback →</a>
        </div>
        <p style="margin: 25px 0 0 0; font-size: 14px; color: #6b7280; text-align: center;">Thank you for being part of Eklan! 🌟</p>
      </div>
    </div>
    <div style="text-align: center; margin-top: 30px; color: #9ca3af; font-size: 12px;">
      <p style="margin: 0;">This is an automated notification from Eklan.</p>
...
Thank you for attending "${data.classTitle}".

We'd love your feedback: ${data.formName}
${data.formUrl}

Thank you for being part of Eklan! 🌟
...
This is an automated notification from Eklan.
    ...
    logger.info('NPS form email sent', {
      studentEmail: data.studentEmail,
      classTitle: data.classTitle,
    ...
    logger.error('Error sending NPS form email', {
      error: msg,
      studentEmail: data.studentEmail,
    });
    throw error;
