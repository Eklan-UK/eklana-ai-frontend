...
const DialogueTurnSchema = new Schema({
	speaker: {
		...
		description: 'Speaker identifier - "student" for student lines, "ai_0", "ai_1", etc. for AI characters',
	},
	text: {
		type: String,
		required: true,
...
const TargetSentenceSchema = new Schema({
	word: {
		type: String,
		default: '',
		description: 'Vocabulary word (used in vocabulary drills for word-level practice)',
	},
	wordTranslation: {
		type: String,
		default: '',
...
export interface IDrill extends Document {
	_id: Types.ObjectId;
	title: string;
	type: 'vocabulary' | 'roleplay' | 'matching' | 'definition' | 'summary' | 'grammar' | 'sentence_writing';
...
const drillSchema = new Schema<IDrill>(
	{
		...
		type: {
			...
			enum: {
				values: ['vocabulary', 'roleplay', 'matching', 'definition', 'summary', 'grammar', 'sentence_writing'],
